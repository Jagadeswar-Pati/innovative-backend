...
function buildInvoicePdfBuffer(order, invoiceNumber) {
  return new Promise((resolve, reject) => {
    ...
    const maxRowsPerPage = Math.floor((tableBottomY - 200 - headerRowH - dataRowH) / dataRowH);
    ...
    try {
      doc.image(logoPath, MARGIN.left, headerY, { height: logoHeight });
    } catch (_) {
      doc.fillColor('#000000').fontSize(14).font('Helvetica-Bold').text(COMPANY.name, MARGIN.left, headerY);
    }
    doc.fillColor('#000000').fontSize(11).font('Helvetica-Bold').text(COMPANY.name, MARGIN.left + logoWidth + 10, headerY + 6);
    let y = headerY + logoHeight + 6;
    ...
    const grandTotal = sumRowTotal + deliveryCharge;
    const boxW = 180;
    ...
    doc.text('CGST Total', boxX + 6, totY + 18);
    doc.text(formatCurrency(sumCGST), boxX + boxW - 50, totY + 18);
    doc.text('SGST Total', boxX + 6, totY + 30);
    ...
    doc.moveTo(boxX, totY + 52).lineTo(boxX + boxW, totY + 52).stroke();
    doc.fillColor('#000000').font('Helvetica-Bold').fontSize(10);
    doc.text('Grand Total', boxX + 6, totY + 62);
    doc.text(formatCurrency(grandTotal), boxX + boxW - 55, totY + 62);
    doc.strokeColor('#374151').rect(boxX, totY, boxW, TOTALS_BOX_HEIGHT).stroke();
